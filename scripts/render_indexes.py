...
then runs this. Dates are computed from calendar days in SOLVED_TZ (below), not
from seconds, so output only changes when the day changes.
...
# Solve dates are rendered in the SOLVER'S timezone, not UTC.
#
# LeetCode stores a submission as a Unix timestamp, and this script used to turn
# that into a calendar day with tz=timezone.utc. For anyone east of UTC that is
# fine; for anyone west of it, every solve after local evening lands on the NEXT
# UTC day. Ayush is US-Eastern, so anything submitted after 20:00 ET was credited
# to tomorrow — 5 of the first 17 problems were stamped a day late, including one
# solved at 20:09 ET that the page dated to the following morning.
#
# Anything user-facing that says "solved on <date>" or "<n> days ago" has to
# agree with the wall clock the problem was actually solved on, so every such
# computation goes through SOLVED_TZ. A fork sets LEETLOOP_TZ to its own zone;
# the name is an IANA identifier, so DST is handled for free and this stays
# correct across the November and March transitions.
SOLVED_TZ_NAME = os.environ.get("LEETLOOP_TZ", "America/New_York")


def _resolve_tz(name: str):
    """SOLVED_TZ, or UTC plus a loud warning if the zone database is missing.

    Falling back silently would quietly reintroduce the exact off-by-one-day bug
    this exists to fix, so the fallback is noisy. It is not fatal, because a
    missing tzdata should not be able to block the pipeline from rendering.
    """
    if ZoneInfo is None:
        print(
            "WARNING: zoneinfo unavailable (Python < 3.9); rendering dates in UTC. "
            "Late-evening solves will be dated a day late.",
            file=sys.stderr,
        )
        return timezone.utc
    try:
        ...
        print(
            "WARNING: timezone %r unavailable (%s); rendering dates in UTC. "
            "Late-evening solves will be dated a day late. Install the `tzdata` "
            "package, or set LEETLOOP_TZ to a zone this system has."
            % (name, exc),
            file=sys.stderr,
        )
        return timezone.utc
...
SOLVED_TZ = _resolve_tz(SOLVED_TZ_NAME)
...
def solved_date(entry) -> str:
    return datetime.fromtimestamp(entry["timestamp"], tz=SOLVED_TZ).strftime("%Y-%m-%d")


def days_since(entry, today) -> int:
    solved = datetime.fromtimestamp(entry["timestamp"], tz=SOLVED_TZ).date()
    ...
    return "\n".join(
        [
            ...
            *rows,
            ...
            "",
            "```bash",
            "git clone https://github.com/%s && cd %s"
...
def render_review_queue(problems) -> str:
    # "Today" must be the same clock the solve dates use, or a problem solved
    # this evening reads as "1 day ago" the moment UTC rolls over.
    today = datetime.now(tz=SOLVED_TZ).date()
    ...
    lines = [
        ...
        "",
        "Generated for %s (%s)." % (today.isoformat(), SOLVED_TZ_NAME),
        "",
    ]
